...
var KanbanView = class extends import_obsidian.BasesView {
  ...
  patchColumnCards(columnEl, newEntries) {
    ...
    if (countEl)
      countEl.textContent = `(${newEntries.length})`;
  ...
  createCard(entry) {
    ...
    for (const propertyId of order) {
      if (propertyId === this.groupByPropertyId)
        continue;
      ...
      if (this.app && valueStr.includes("[[")) {
        void import_obsidian.MarkdownRenderer.render(this.app, valueStr, valueEl, filePath, this);
      } else {
  ...
  openColorPicker(anchorEl, columnEl, columnValue) {
    ...
    if (!currentColor)
      noneSwatch.classList.add(CSS_CLASSES.COLUMN_COLOR_SWATCH_ACTIVE);
    ...
    for (const color of COLOR_PALETTE) {
      ...
      if (currentColor === color.name)
        swatch.classList.add(CSS_CLASSES.COLUMN_COLOR_SWATCH_ACTIVE);
    ...
    const sortable = new sortable_esm_default(body, {
      ...
      onStart: (evt) => {
        ...
        if (evt.item instanceof HTMLElement)
          evt.item.classList.remove(CSS_CLASSES.CARD_HOVER);
      },
      onEnd: (evt) => {
        this._dragging = false;
  ...
  async handleCardDrop(evt) {
    ...
    if (oldColumnEl instanceof HTMLElement && oldColumnValue) {
      ...
      if (oldBody)
        this._prefs.cardOrders[oldColumnValue] = getColumnPaths(oldBody);
    }
    this._prefs.cardOrders[newColumnValue] = getColumnPaths(evt.to);
    this._persistPrefs();
